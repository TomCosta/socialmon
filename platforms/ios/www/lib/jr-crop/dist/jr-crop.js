...
function($ionicModal, $rootScope, $q) {
  ...
  var template = '<div class="jr-crop modal">' +
                    '<ion-header-bar class="bar bar-header bar-positive">' +
                      '<h1 class="title">Crop Picture</h1>' +
                    '</ion-header-bar>' +
                    '<ion-content style="position:absolute; background-color:black;">' + 
                      '<div class="jr-crop-center-img-container">' +
                        '<div class="jr-crop-img" ng-style="{width: width + \'px\', height: height + \'px\'}" style="margin: 0 auto; opacity: 0.6;"></div>' +
                      '</div>' +
                      '<div class="jr-crop-center-cropped-container">' +
                        '<div class="jr-crop-select" ng-class="{\'jr-crop-select-circle\': circle}" style="overflow: hidden; margin-left:auto; margin-right:auto; border: 2px solid white;" ng-style="{width: width + \'px\', height: height + \'px\'}"></div>' +
                      '</div>' +
                      '<div class="item item-dark" style="position:absolute; bottom:50px; opacity:0.6; width:100%;">' +
                        '<span>* Slide up/down to crop your image!</span>' + 
                      '</div>' +
                    '</ion-content>' + 
  ...
  var jrCropController = ionic.views.View.inherit({
    ...
    initImage: function() {
      if (this.options.height < this.imgHeight || this.options.width < this.imgWidth) {
        ...
        // console.log("selectAspectRatio: " + selectAspectRatio);
    ...
    bindHandlers: function() {
      var self = this,
          ...
          // console.log("self.imgWidth: " + self.imgWidth);
          // console.log("self.imgHeight: " + self.imgHeight);
          // console.log("image_ratio: " + image_ratio);
          // console.log("self.options.width" + self.options.width);
          // console.log("self.options.height" + self.options.height);
      ...
      function calcMaxPos() {
        ...
        // console.log("min_pos_y: " + min_pos_y);
    ...
    loadImage: function() {
      ...
      angular.element('<img style="top:20%; margin:0 auto;"/>')
